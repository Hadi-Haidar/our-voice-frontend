...
export default function Issues() {
  ...
  // ── derived list ────────────────────────────────────────────────────────────
  const filtered = issuesMock.filter((issue) => {
    const matchCat = activeCategories.length === 0 || activeCategories.includes(issue.category);
    const matchStatus = activeStatus === "all" || issue.status === activeStatus;
    const matchSearch = search === "" ||
      issue.title.includes(search) ||
      issue.location.includes(search) ||
      issue.description.includes(search);
  ...
  const statusLabel = (s, ar) => {
    ...
    if (s === "open") return ar ? "قيد الانتظار" : "Pending";
    if (s === "resolved") return ar ? "تم الحل" : "Resolved";
  };
          ...
          {(() => {
            ...
            return displayedCats.map((cat) => {
              ...
              return (
                ...
                >
                  <cat.icon className={`h-4 w-4 ${isActive ? "text-white" : "text-gray-400 group-hover:text-red-600"}`} />
              ...
              filtered.map((issue, index) => (
                ...
                >
                  ...
                  {(issue.image || issue.isVideo) && (
                    <div className="h-48 overflow-hidden relative shrink-0 rounded-t-2xl">
                      ...
                      {issue.image && (
                        <img
                          alt={issue.title}
                          className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ${issue.status === "resolved" ? "grayscale opacity-80" : ""
                            }`}
                          src={issue.image}
                        />
                      )}
                      <span
                        className={`absolute top-3 ${isRTL ? "right-3" : "left-3"
                          } ${issue.categoryColor} text-xs font-semibold px-2.5 py-0.5 rounded-full z-20`}
                      >
                        {issue.category}
                      </span>
                    </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    {!issue.image && !issue.isVideo && (
                      <span className={`${issue.categoryColor} text-xs font-semibold px-2.5 py-0.5 rounded-full mb-3 self-start`}>
                        {issue.category}
                      </span>
                    )}
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-400 mb-4 flex-wrap">
                      ...
                      {issue.coordinates && (
                        <span className="text-[10px] font-bold text-red-600 bg-red-50 dark:bg-red-900/20 px-1 rounded border border-red-100 dark:border-red-800">GPS</span>
                      )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {CATEGORIES.map((cat) => {
                      ...
                      return (
                        ...
                        >
                          <cat.icon className={`h-3 w-3 ${isActive ? "text-white" : "text-gray-400"}`} />
                          <span>{isRTL ? cat.labelAr : cat.labelEn}</span>
                        </button>
                      );
                    })}
